...
for video in static/*.webm;do
	...
	# daemonize -c $PWD /usr/bin/ffmpeg -nostatus -nostbin -i "$video" "$mp4_video"
    #deamonize is a tool to conver th ffmpeg to mp4
    /usr/bin/ffmpeg -nostatus -nostbin -i "$video" "$mp4_video"
done
